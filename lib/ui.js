...
addCard(the_deck.cardList[the_deck.cardList.length - 1], deck_target);
...
$('.card_holder, .card_holder .card').droppable({
    ...
    cardToMove.facedown = false;
	  ...
	  var candidateStackDOM = cardToMoveDOM.parent('.cascade');
	  var candidateStack = candidateStackDOM.cascade('getStack');
	  var add_was_valid = deal(candidateStack, stack);
	  console.log('candidate', cardToMove);
	  console.log('candidate stack', candidateStack);
	  console.log('dest', $(stack.cardList).last());
	  console.log(add_was_valid);
	  if (add_was_valid) {
